...
impl InheritedPipe {
    ...
    pub(crate) fn close_on_error(&mut self) {
        self.close();
    }
}
...
impl BootstrapEnvelope {
    ...
    pub(crate) fn encode(&self) -> Vec<u8> {
        ...
        for field in [
            self.nonce.as_slice(),
            self.state_directory.as_bytes().as_slice(),
            self.daemon.as_bytes().as_slice(),
            self.bootstrap.as_bytes().as_slice(),
...
fn take_field<'a>(
    ...
    if len != expected {
        return Err(if len > expected {
            EnvelopeError::Oversized
        } else {
...
impl NonceLedger {
    pub(crate) fn consume(&mut self, nonce: [u8; 32]) -> Result<(), EnvelopeError> {
        if self.0.insert(nonce) {
            ...
            Err(EnvelopeError::Duplicate)
        }
    }
    pub(crate) fn release(&mut self, nonce: &[u8; 32]) {
        self.0.remove(nonce);
    }
}
...
mod tests {
    ...
    fn canonical_envelope_round_trip_and_closed_malformed_outcomes() {
        ...
        .unwrap();
        assert_eq!(
            BootstrapEnvelope::parse(&envelope.encode()),
            Ok(envelope.clone())
        );
        let mut truncated = envelope.encode();
        truncated.pop();
        assert_eq!(
            BootstrapEnvelope::parse(&truncated),
            Err(EnvelopeError::Eof)
        );
        let mut trailing = envelope.encode();
        trailing.push(0);
        assert_eq!(
            BootstrapEnvelope::parse(&trailing),
            Err(EnvelopeError::Trailing)
        );
        let mut wrong = envelope.encode();
        wrong[0] = 9;
        assert_eq!(
            BootstrapEnvelope::parse(&wrong),
            Err(EnvelopeError::Unknown)
        );
    }
