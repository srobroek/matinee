macro_rules! foundation_contract_tests {
    () => {
        ...
        use crate::test_support_channel::{
            RecordingSink, establish_pair, establish_pair_at_epochs, owned_operation,
        };
        ...
        fn foundational_boundary_accepts_valid_session_and_rejects_mutations() {
            ...
            let output =
                crate::AuthorizedOutput::filtered(crate::PayloadKind::Command, b"payload".to_vec())
                    .unwrap();
            ...
            // A session cannot be driven at an epoch it did not authenticate: the epoch is
            // the principal's, so a client that believes in a superseded epoch is refused
            // before any traffic key exists.
        ...
        fn payload_bounds_and_debug_redaction_are_observable() {
            ...
            let frame = client
                .send(
                    &crate::AuthorizedOutput::filtered(
                        crate::PayloadKind::Event,
            ...
            let input = daemon
                .receive(
                    &frame,
                    &owned_operation(crate::PayloadKind::Event),
                    &mut sink,
                )
                .unwrap();
            ...
            // A peer that ignores the declared bound is rejected by the receiver, before the
            // payload reaches a typed value and before the receive counter moves.
            let (mut client, mut daemon) = establish_pair(7);
            let oversized = client
                .seal_unbounded(
                    crate::PayloadKind::StreamChunk,
                    &vec![0u8; crate::MAX_CHUNK_BYTES + 1],
                )
                .unwrap();
            let failure = daemon
                .receive(
                    &oversized,
                    &owned_operation(crate::PayloadKind::StreamChunk),
                    &mut sink,
                )
                .expect_err("payload past the declared chunk bound");
            assert_eq!(failure.code(), crate::FailureCode::ResourceLimit);
        ...
        fn identity_reference_shapes_and_bounds_are_closed() {
            ...
            assert!(
                crate::identity::PublicKey::from_uncompressed(
                    [3; crate::identity::UNCOMPRESSED_KEY_BYTES]
                )
                .is_err()
            );
            ...
            let reference =
                crate::identity::CredentialReference::new("keychain", "daemon/key", daemon, daemon)
                    .unwrap();
            ...
            assert!(
                crate::identity::CredentialReference::new("p", "k".repeat(257), daemon, daemon)
                    .is_err()
            );
        }
        ...
        fn every_adapter_closed_outcome_remains_distinct_and_opaque() {
            ...
            for error in [
                ...
                crate::adapters::credential_store::CredentialStoreError::Unavailable,
            ] {
                assert_eq!(
                    FakeCredentialStore::new()
                        .with_error(binding, error)
                        .lookup(binding),
                    Err(error)
                );
            }
            for error in [
                crate::adapters::os_pipe::OsPipeError::Missing,
                crate::adapters::os_pipe::OsPipeError::Mismatch,
                crate::adapters::os_pipe::OsPipeError::Duplicate,
                crate::adapters::os_pipe::OsPipeError::Unavailable,
                crate::adapters::os_pipe::OsPipeError::Malformed,
                crate::adapters::os_pipe::OsPipeError::Oversized,
            ] {
            ...
            assert_eq!(
                format!(
                    "{:?}",
                    crate::adapters::credential_store::CredentialHandle::from_slot(9)
                ),
            ...
            assert_eq!(
                format!(
                    "{:?}",
                    crate::adapters::os_pipe::InheritedPipe::from_handle(9)
                ),
        ...
        fn commands_are_closed_idempotent_and_operation_mapped() {
            ...
            let commands = [
                ...
                crate::SecurityCommand::CreateEnrollment {
                    enrollment: transition,
                    daemon: id,
                    idempotency: key(2),
                },
                crate::SecurityCommand::ConsumeEnrollment {
                    enrollment: transition,
                    idempotency: key(3),
                },
                crate::SecurityCommand::Rotate {
                    principal: id,
                    new_fingerprint: crate::identity::Fingerprint::new("a".repeat(64)).unwrap(),
                    idempotency: key(4),
                },
                crate::SecurityCommand::Revoke {
                    principal: id,
                    idempotency: key(5),
                },
            ];
            let expected = [
                crate::identity::TransitionOperation::Bootstrap,
                crate::identity::TransitionOperation::EnrollmentCreate,
                crate::identity::TransitionOperation::EnrollmentConsume,
                crate::identity::TransitionOperation::Rotation,
                crate::identity::TransitionOperation::Revocation,
            ];
