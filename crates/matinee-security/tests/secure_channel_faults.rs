macro_rules! secure_channel_faults_tests {
    () => {
        ...
        fn fault_connection(epoch: u64) -> Connection {
            ...
            connection.authenticate().expect("authenticated fault fixture");
            connection
        }
        ...
        fn reject_before_dispatch(
            ...
            let failure = AuthorizedInput::authorized(context, payload).expect_err("fault input");
            assert_eq!(failure.code(), expected);
            session.close();
            assert_eq!(
                session.binds(context).expect_err("closed before dispatch").code(),
        ...
        fn faults_reject_disjoint_substituted_and_downgraded_contracts() {
            ...
            assert_eq!(contract(1, 1, 2), Err(FailureCode::CompatibilityUnsupported));
            assert_eq!(contract(1, 3, 0), Err(FailureCode::CompatibilityUnsupported));
        }
        ...
        fn faults_close_before_dispatch_for_endpoint_epoch_nonce_signature_and_tag() {
            ...
            session.close();
            assert_eq!(session.binds(&context).unwrap_err().code(), FailureCode::AuthenticationFailed);
        }
