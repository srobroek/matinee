...
Releases ownership. `close_tab` is optional and defaults to false. Returns
`reconciliation_required` while an Unknown Reservation quarantines the target
(`FR-044`).
...
restores the previously active tab, and persists one bounded redacted artifact
(`FR-030`, `SC-019`).
...
| `daemon.stopped_before_dispatch` | Stop or recovery cancelled undispatched work |
| `daemon.stop_blocked` | An `uncertain` Operation blocks clean stop |
| `operation.uncertain` | The extension cannot prove the outcome |
| `reconciliation_required` | A dispatched effect has no authoritative result |
...
| `storage.unavailable` | The store is unavailable or corrupt |
