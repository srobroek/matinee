...
- **FR-055**: The repository MUST provide one deterministic fixture with two independently observable tab routes, editable fields, increment controls, manually established authenticated state, and a stall control that holds a dispatched effect unresolved so a lost boundary can be injected.
- **FR-056**: The implementation MUST provide `specs/006.5-demonstrable-browser-mvp/quickstart.md`. That procedure MUST state exact prerequisites, invocations, expected boundary results, restart steps, lost-boundary injection, and cleanup for the repeatable demonstration.
