...
├── matinee-runtime/               # existing; owns the process-lifetime enrollment host that consumes matinee-security
...
|---|---|
| Architecture: current workspace | Add `crates/matinee-security` to the root `Cargo.toml` members, create its crate manifest, and regenerate the root `Cargo.lock` with Cargo; the CLI surface remains unchanged, and `matinee-runtime` gains only the process-lifetime enrollment host that makes the security boundary reachable from production code. |
