---
name: speckit-status-report-show
...
metadata:
  ...
  source: status-report:commands/show.md
---
...
```
.specify/extensions/status-report/scripts/bash/get-project-status.sh --json
```
...
- `$speckit-clarify` — spec exists with no clarifications recorded
- `$speckit-checklist` — no `checklists/` for a feature that has a plan
- `$speckit-analyze` — tasks exist and have not been analyzed
- `$speckit-converge` — tasks read 100% complete but the feature is not yet merged, to catch unbuilt work
